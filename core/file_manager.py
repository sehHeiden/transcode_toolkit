...
class FileManager:
    ...
    def __init__(self, backup_strategy: BackupStrategy = BackupStrategy.ON_SUCCESS, retention_days: int = 0):
    ...
    def create_backup(self, file_path: Path) -> Optional[Path]:
        ...
        try:
            ...
            raise ProcessingError(f"Backup creation failed: {e}", file_path=file_path, cause=e)
    ...
    def atomic_replace(self, source_path: Path, temp_path: Path, create_backup: bool = True) -> FileOperation:
        ...
        try:
            ...
            final_path = source_path.with_suffix(".opus") if source_path.suffix.lower() not in {".opus"} else source_path
            ...
            if backup_path and self.backup_strategy in {BackupStrategy.IMMEDIATE, BackupStrategy.ON_SUCCESS}:
            ...
            raise ProcessingError(f"Atomic file replacement failed: {e}", file_path=source_path, cause=e)
    ...
    def cleanup_session_backups(self) -> int:
        ...
        if self.backup_strategy not in {BackupStrategy.SESSION_END, BackupStrategy.ON_SUCCESS}:
            return 0
        ...
        for backup_file in backup_files:
            try:
                ...
                if file_age > retention_seconds:
                    backup_file.unlink()
                    cleaned_count += 1
                    LOG.info(f"Removed old backup (age: {file_age/86400:.1f} days): {backup_file}")
            except Exception as e:
