...
def _estimate_with_snr(file_path: Path, meta: dict[str, Any], preset_config) -> int:
    """Estimate size using SNR-based bitrate calculation."""
    from core import FFmpegProbe
    ...
    if not preset_config.snr_bitrate_scale or preset_config.min_snr_db is None:
        ...
        try:
            ...
            LOG.warning("SNR estimation failed for %s: %s", file_path, e)
            effective_bitrate_bps = target_bitrate_bps

    return int(meta["duration"] * effective_bitrate_bps / 8)
...
def analyse(root: Path, target_br: int) -> list[tuple[Path, int, int]]:
    ...
    max_workers = min(psutil.cpu_count(logical=True), len(files))
...
def compare_presets(root: Path) -> list[EstimationResult]:
    ...
    max_workers = min(8, psutil.cpu_count(logical=False), len(all_files))  # Cap at 8, use physical cores
